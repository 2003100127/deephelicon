...
class stage2_Graph_Call(object):

    def __init__(self, ):
        self.dataparser = dataInitializer_dhc_rs()
    ...
    def reborn(self, ):
        ...
        prediction = tf_graph.get_tensor_by_name("sigmoid:0")
        print(
            "Protein: {}".format(sys.argv[1] + sys.argv[2])
        )
        accumulator = []
        x_test, _, num_test_samples = self.dataparser.input2d(
        ...
        for batch in range(num_batch_test + 1):
            if batch < num_batch_test:
                x_batch_te, _ = self.dataparser.batchData(
